...
class Box2BoxTransform(object):
    "box2box used in HumanLiker the second step"
    def __init__(
    ...
    def get_deltas(self, src_boxes, target_boxes):
        ...
        src_tl_x = src_boxes[:, 0]
        src_tl_y = src_boxes[:, 1]

        target_widths = target_boxes[:, 2] - target_boxes[:, 0]
        target_heights = target_boxes[:, 3] - target_boxes[:, 1]

        target_tl_x = target_boxes[:, 0]
        target_tl_y = target_boxes[:, 1]

        wx, wy, ww, wh = self.weights
        dx = wx * (target_tl_x - src_tl_x) / src_widths
        dy = wy * (target_tl_y - src_tl_y) / src_heights
    ...
    def apply_deltas(self, deltas, boxes):
        ...
        pred_tl_x = dx * widths[:, None] + tl_x[:, None]
        pred_tl_y = dy * heights[:, None] + tl_y[:, None]
