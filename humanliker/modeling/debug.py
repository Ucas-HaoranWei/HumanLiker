            ...
            for j in range(len(preds[i].scores) if preds is not None else 0):
                if preds[i].scores[j] > vis_thresh:
                    ...
                    cv2.rectangle(
                        ...
                        (int(bbox[2]), int(bbox[3])), 
                        (int(cl[0]), int(cl[1]), int(cl[2])), 5, cv2.LINE_AA)
...
def debug_second_stage(images, instances, proposals=None, vis_thresh=0.5,
        ...
        for j in range(len(bboxes)):
            if scores[j] > vis_thresh:
                ...
                cv2.rectangle(
                    ...
                    (int(bbox[2]), int(bbox[3])),
                    cl, 3, cv2.LINE_AA)
                cv2.circle(image, (int(bbox[0]), int(bbox[1])), 6, (255,0,0), -1)
                cv2.circle(image, (int(bbox[0]), int(bbox[1])), 7, (0, 0, 0), 2)
                # if debug_show_name:
                #     cat = cats[j]
                #     txt = '{}{:.1f}'.format(
                #         cat2name[cat] if cat > 0 else '',
